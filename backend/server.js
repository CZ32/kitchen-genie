...
app
  .use(cors({ allowHeaders: headersWhiteList, allowCredentials: true, allowOrigins: Deno.env.get("ALLOWED_ORIGINS").split(', ') }))
  ...
  .get('/myrecipes', async (server) => {
    ...
    if (currentUser) {
      ...
      // Bad Credentials
      await server.json({ response: 'unauthorized', recipes: [], loggedInUser: {} })
    }
